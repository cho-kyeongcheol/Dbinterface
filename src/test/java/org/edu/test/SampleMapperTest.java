...
@WebAppConfiguration //JUnit과 AOP동시사용 에러처리를 위해
...
public class SampleMapperTest {
	...
	* 프로젝트폴더경로 DbInterface/src/main/webapp/WEB-INF/spring/root-context.xml (Mysql서버접속정보 변경해야 합니다.)
	...
	@Test
		...
		testSelectMember();
		System.out.println("수정전이름");
			MemberVO vo = new MemberVO();
			//수정은 여러개의 변수값을 변경하기 때문에 MemberVO클래스 변수를 매개변수로 사용한다.
			vo.setUserid("user");
			vo.setUserpw("43211");
			vo.setUsername("공주님1");
			vo.setEmail("abc@abc.com1");
			mapper.updateMember(vo);
			
			//System.out.println("아래는 수정 전 이름을 확인하세요");
			//testSelectMember();
	}
